...
public class NaNoRepeatSubmitIPAOP {
    ...
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        ...
        // 构建唯一请求的Key
        String key = ip + authentication + "-" + request.getServletPath();

        // 获取注解上的过期时间，如果没有设置则使用默认值
        NaNoRepeatSubmitIP annotation = ((MethodSignature) pjp.getSignature()).getMethod().getAnnotation(NaNoRepeatSubmitIP.class);
        ...
        String dateStartMsg = annotation.dateStartMsg();
        String dateEndMsg = annotation.dateEndMsg();
...
//        String zoneId = annotation.zoneId();
        ...
        if(requestParams){
            ...
            // 构建唯一请求的Key
            key = ip + authentication + "-" + request.getServletPath() + "-" + stringBuilder;
        }
            ...
            if (cacheNum >= num) {
                ...
                if (showRemainingTime) {
                    if (remainingExpireTime != null && remainingExpireTime > 0) {
                        msg = msg + dateStartMsg + NaDateTimeUtil.parseToString(remainingExpireTime, dateFormat) + dateEndMsg;
                    }
                }
